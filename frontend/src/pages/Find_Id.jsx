...
import Complete_Find_Id from "../popup/Complete_Find_Id.jsx";
...
const Find_Id = () => {
    ...
    const [iscompleteFindId, setcompleteFindId] = useState(false);
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Axios를 이용한 POST 요청
            const response = await axios.post("http://localhost:8080/api/findUserId", {
                name: formData.name,
            ...
            if (response.data.success) {
                ...
                // userId를 formData에 추가
                setFormData({ ...formData, userId: userId });

                // 성공 모달 열기
                setcompleteFindId(true);

                console.log("찾은 아이디:", userId); // 서버 응답 데이터 출력
            } else {
                alert("이름과 전화번호가 일치하는 아이디가 없습니다.");
            }
        } catch (error) {
...
<div>
    < Complete_Find_Id
        isOpen={iscompleteFindId}
        onClose={() => setcompleteFindId(false)} // 팝업 닫기
        userId={formData.userId}
    />
    <div className="flex flex-col items-center w-full px-4 mt-[3rem]">
        <div className="flex flex-row items-center justify-center">
            <a className="text-red-400 text-xl  cursor-pointer font-bold mt-8 mb-6">아이디 찾기</a>
            <h1 className="text-red-400 text-xl font-bold mt-8 mb-6 ml-[2rem] mr-[2rem]">/</h1>
            <a onClick={handlegoFindPw} className="text-gray-400 text-xl  cursor-pointer font-bold mt-8 mb-6">비밀번호 찾기</a>
        </div>
